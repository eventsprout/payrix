<?php

declare(strict_types=1);

/*
 * Payrix
 *
 * This file was automatically generated by Worldpay.
 */

namespace PayrixLib\Models;

use PayrixLib\ApiHelper;
use stdClass;

class SecretsPostRequest implements \JsonSerializable
{
    /**
     * @var string
     */
    private $login;

    /**
     * @var string|null
     */
    private $entity;

    /**
     * @var string|OrgsPostRequest|null
     */
    private $org;

    /**
     * @var string|null
     */
    private $division;

    /**
     * @var string|null
     */
    private $partition;

    /**
     * @var string
     */
    private $type;

    /**
     * @var string|null
     */
    private $platform;

    /**
     * @var string|null
     */
    private $name;

    /**
     * @var string|null
     */
    private $description;

    /**
     * @var string
     */
    private $key;

    /**
     * @var int|null
     */
    private $locked = SecretLockedEnum::NOTLOCKED;

    /**
     * @var int|null
     */
    private $inactive = InactiveEnum::ACTIVE;

    /**
     * @var int|null
     */
    private $frozen = FrozenEnum::NOTFROZEN;

    /**
     * @param string $login
     * @param string $type
     * @param string $key
     */
    public function __construct(string $login, string $type, string $key)
    {
        $this->login = $login;
        $this->type = $type;
        $this->key = $key;
    }

    /**
     * Returns Login.
     * The identifier of the Login that owns this secret resource.
     */
    public function getLogin(): string
    {
        return $this->login;
    }

    /**
     * Sets Login.
     * The identifier of the Login that owns this secret resource.
     *
     * @required
     * @maps login
     */
    public function setLogin(string $login): void
    {
        $this->login = $login;
    }

    /**
     * Returns Entity.
     * The identifier of the Entity associated with this secret.
     */
    public function getEntity(): ?string
    {
        return $this->entity;
    }

    /**
     * Sets Entity.
     * The identifier of the Entity associated with this secret.
     *
     * @maps entity
     */
    public function setEntity(?string $entity): void
    {
        $this->entity = $entity;
    }

    /**
     * Returns Org.
     * The identifier of the Org associated with this secret.
     *
     * @return string|OrgsPostRequest|null
     */
    public function getOrg()
    {
        return $this->org;
    }

    /**
     * Sets Org.
     * The identifier of the Org associated with this secret.
     *
     * @maps org
     * @mapsBy anyOf(anyOf(string,OrgsPostRequest),null)
     *
     * @param string|OrgsPostRequest|null $org
     */
    public function setOrg($org): void
    {
        $this->org = $org;
    }

    /**
     * Returns Division.
     * The identifier of the Division associated with this secret.
     */
    public function getDivision(): ?string
    {
        return $this->division;
    }

    /**
     * Sets Division.
     * The identifier of the Division associated with this secret.
     *
     * @maps division
     */
    public function setDivision(?string $division): void
    {
        $this->division = $division;
    }

    /**
     * Returns Partition.
     * The identifier of the Partition associated with this secret.
     */
    public function getPartition(): ?string
    {
        return $this->partition;
    }

    /**
     * Sets Partition.
     * The identifier of the Partition associated with this secret.
     *
     * @maps partition
     */
    public function setPartition(?string $partition): void
    {
        $this->partition = $partition;
    }

    /**
     * Returns Type.
     * The type of secret.
     *
     * <details>
     * <summary>Valid Values</summary>
     *
     * - `baseKey` - **Base Derivation Key.**
     * - `sftp` - **SFTP Server Key.**
     * - `publicKey` - **Public Key.**
     * - `privateKey` - **Private Key.**
     * - `ecPrivateKey` - **Elliptic Curve Private Key.**
     * - `apikey` - **API Key.**
     * - `oldPrivateKey` - **Expired Private Key.**
     *
     * </details>
     */
    public function getType(): string
    {
        return $this->type;
    }

    /**
     * Sets Type.
     * The type of secret.
     *
     * <details>
     * <summary>Valid Values</summary>
     *
     * - `baseKey` - **Base Derivation Key.**
     * - `sftp` - **SFTP Server Key.**
     * - `publicKey` - **Public Key.**
     * - `privateKey` - **Private Key.**
     * - `ecPrivateKey` - **Elliptic Curve Private Key.**
     * - `apikey` - **API Key.**
     * - `oldPrivateKey` - **Expired Private Key.**
     *
     * </details>
     *
     * @required
     * @maps type
     * @factory \PayrixLib\Models\SecretTypeEnum::checkValue
     */
    public function setType(string $type): void
    {
        $this->type = $type;
    }

    /**
     * Returns Platform.
     * The platform used to process this Transaction.
     *
     * <details>
     * <summary>Valid Values</summary>
     *
     * - `APPLE` - **Apple Payment Processor**
     * - `ELAVON` - **ELAVON Payment Processor**
     * - `FIRSTDATA` - **FirstData Payment Processor**
     * - `GOOGLE` - **Google Payment Processor**
     * - `VANTIV` - **WorldPay / Vantiv eComm Payment Processor (VAP)**
     * - `VCORE` - **WorldPay / Vantiv Core Payment Processor**
     * - `WELLSACH` - **Wells Fargo Merchant Services Payment Processor (ACH)**
     * - `WELLSFARGO` - **Wells Fargo Merchant Services Payment Processor**
     * - `WFSINGLE` - **Wells Fargo Single Payment Processor**
     * - `WORLDPAY` - **WORLDPAY**
     * - `TDBANKCA` - **TDBANKCA**
     * </details>
     */
    public function getPlatform(): ?string
    {
        return $this->platform;
    }

    /**
     * Sets Platform.
     * The platform used to process this Transaction.
     *
     * <details>
     * <summary>Valid Values</summary>
     *
     * - `APPLE` - **Apple Payment Processor**
     * - `ELAVON` - **ELAVON Payment Processor**
     * - `FIRSTDATA` - **FirstData Payment Processor**
     * - `GOOGLE` - **Google Payment Processor**
     * - `VANTIV` - **WorldPay / Vantiv eComm Payment Processor (VAP)**
     * - `VCORE` - **WorldPay / Vantiv Core Payment Processor**
     * - `WELLSACH` - **Wells Fargo Merchant Services Payment Processor (ACH)**
     * - `WELLSFARGO` - **Wells Fargo Merchant Services Payment Processor**
     * - `WFSINGLE` - **Wells Fargo Single Payment Processor**
     * - `WORLDPAY` - **WORLDPAY**
     * - `TDBANKCA` - **TDBANKCA**
     * </details>
     *
     * @maps platform
     * @factory \PayrixLib\Models\SecretsPlatformEnum::checkValue
     */
    public function setPlatform(?string $platform): void
    {
        $this->platform = $platform;
    }

    /**
     * Returns Name.
     * The name of this Secret.
     * This field is stored as a text string and must be between 0 and 100 characters long.
     */
    public function getName(): ?string
    {
        return $this->name;
    }

    /**
     * Sets Name.
     * The name of this Secret.
     * This field is stored as a text string and must be between 0 and 100 characters long.
     *
     * @maps name
     */
    public function setName(?string $name): void
    {
        $this->name = $name;
    }

    /**
     * Returns Description.
     * A description of this Secret.
     * This field is stored as a text string and must be between 0 and 500 characters long.
     */
    public function getDescription(): ?string
    {
        return $this->description;
    }

    /**
     * Sets Description.
     * A description of this Secret.
     * This field is stored as a text string and must be between 0 and 500 characters long.
     *
     * @maps description
     */
    public function setDescription(?string $description): void
    {
        $this->description = $description;
    }

    /**
     * Returns Key.
     * The actual key or the indicator of which key to use.
     * This field is stored as a text string and must be between 1 and 5000 characters long.
     */
    public function getKey(): string
    {
        return $this->key;
    }

    /**
     * Sets Key.
     * The actual key or the indicator of which key to use.
     * This field is stored as a text string and must be between 1 and 5000 characters long.
     *
     * @required
     * @maps key
     */
    public function setKey(string $key): void
    {
        $this->key = $key;
    }

    /**
     * Returns Locked.
     * Whether this secret is locked or not.
     * If it is locked, then the value of this type of secret will prevail.
     *
     * <details>
     * <summary>Valid Values</summary>
     *
     * - `0` - **Not Locked**
     * - `1` - **Locked**
     *
     * </details>
     */
    public function getLocked(): ?int
    {
        return $this->locked;
    }

    /**
     * Sets Locked.
     * Whether this secret is locked or not.
     * If it is locked, then the value of this type of secret will prevail.
     *
     * <details>
     * <summary>Valid Values</summary>
     *
     * - `0` - **Not Locked**
     * - `1` - **Locked**
     *
     * </details>
     *
     * @maps locked
     * @factory \PayrixLib\Models\SecretLockedEnum::checkValue
     */
    public function setLocked(?int $locked): void
    {
        $this->locked = $locked;
    }

    /**
     * Returns Inactive.
     * Whether this resource is marked as inactive.
     *
     * <details>
     * <summary>Valid Values</summary>
     *
     * - `0` - **Active**
     * - `1` - **Inactive**
     *
     * </details>
     */
    public function getInactive(): ?int
    {
        return $this->inactive;
    }

    /**
     * Sets Inactive.
     * Whether this resource is marked as inactive.
     *
     * <details>
     * <summary>Valid Values</summary>
     *
     * - `0` - **Active**
     * - `1` - **Inactive**
     *
     * </details>
     *
     * @maps inactive
     * @factory \PayrixLib\Models\InactiveEnum::checkValue
     */
    public function setInactive(?int $inactive): void
    {
        $this->inactive = $inactive;
    }

    /**
     * Returns Frozen.
     * Whether this resource is marked as frozen.
     *
     * <details>
     * <summary>Valid Values</summary>
     *
     * - `0` - **Not Frozen**
     * - `1` - **Frozen**
     *
     * </details>
     */
    public function getFrozen(): ?int
    {
        return $this->frozen;
    }

    /**
     * Sets Frozen.
     * Whether this resource is marked as frozen.
     *
     * <details>
     * <summary>Valid Values</summary>
     *
     * - `0` - **Not Frozen**
     * - `1` - **Frozen**
     *
     * </details>
     *
     * @maps frozen
     * @factory \PayrixLib\Models\FrozenEnum::checkValue
     */
    public function setFrozen(?int $frozen): void
    {
        $this->frozen = $frozen;
    }

    /**
     * Converts the SecretsPostRequest object to a human-readable string representation.
     *
     * @return string The string representation of the SecretsPostRequest object.
     */
    public function __toString(): string
    {
        return ApiHelper::stringify(
            'SecretsPostRequest',
            [
                'login' => $this->login,
                'entity' => $this->entity,
                'org' => $this->org,
                'division' => $this->division,
                'partition' => $this->partition,
                'type' => $this->type,
                'platform' => $this->platform,
                'name' => $this->name,
                'description' => $this->description,
                'key' => $this->key,
                'locked' => $this->locked,
                'inactive' => $this->inactive,
                'frozen' => $this->frozen
            ]
        );
    }

    /**
     * Encode this object to JSON
     *
     * @param bool $asArrayWhenEmpty Whether to serialize this model as an array whenever no fields
     *        are set. (default: false)
     *
     * @return array|stdClass
     */
    #[\ReturnTypeWillChange] // @phan-suppress-current-line PhanUndeclaredClassAttribute for (php < 8.1)
    public function jsonSerialize(bool $asArrayWhenEmpty = false)
    {
        $json = [];
        $json['login']           = $this->login;
        if (isset($this->entity)) {
            $json['entity']      = $this->entity;
        }
        if (isset($this->org)) {
            $json['org']         =
                ApiHelper::getJsonHelper()->verifyTypes(
                    $this->org,
                    'anyOf(anyOf(string,OrgsPostRequest),null)'
                );
        }
        if (isset($this->division)) {
            $json['division']    = $this->division;
        }
        if (isset($this->partition)) {
            $json['partition']   = $this->partition;
        }
        $json['type']            = SecretTypeEnum::checkValue($this->type);
        if (isset($this->platform)) {
            $json['platform']    = SecretsPlatformEnum::checkValue($this->platform);
        }
        if (isset($this->name)) {
            $json['name']        = $this->name;
        }
        if (isset($this->description)) {
            $json['description'] = $this->description;
        }
        $json['key']             = $this->key;
        if (isset($this->locked)) {
            $json['locked']      = SecretLockedEnum::checkValue($this->locked);
        }
        if (isset($this->inactive)) {
            $json['inactive']    = InactiveEnum::checkValue($this->inactive);
        }
        if (isset($this->frozen)) {
            $json['frozen']      = FrozenEnum::checkValue($this->frozen);
        }

        return (!$asArrayWhenEmpty && empty($json)) ? new stdClass() : $json;
    }
}
