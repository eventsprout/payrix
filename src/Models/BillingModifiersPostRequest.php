<?php

declare(strict_types=1);

/*
 * Payrix
 *
 * This file was automatically generated by Worldpay.
 */

namespace PayrixLib\Models;

use PayrixLib\ApiHelper;
use stdClass;

class BillingModifiersPostRequest implements \JsonSerializable
{
    /**
     * @var string
     */
    private $billing;

    /**
     * @var string|null
     */
    private $entity;

    /**
     * @var string|null
     */
    private $org;

    /**
     * @var string|null
     */
    private $division;

    /**
     * @var string|null
     */
    private $partition;

    /**
     * @var string
     */
    private $fromentity;

    /**
     * @var int|null
     */
    private $inactive = InactiveEnum::ACTIVE;

    /**
     * @var int|null
     */
    private $frozen = FrozenEnum::NOTFROZEN;

    /**
     * @param string $billing
     * @param string $fromentity
     */
    public function __construct(string $billing, string $fromentity)
    {
        $this->billing = $billing;
        $this->fromentity = $fromentity;
    }

    /**
     * Returns Billing.
     * The identifier of the Billing that this Billing Modifier applies to.
     */
    public function getBilling(): string
    {
        return $this->billing;
    }

    /**
     * Sets Billing.
     * The identifier of the Billing that this Billing Modifier applies to.
     *
     * @required
     * @maps billing
     */
    public function setBilling(string $billing): void
    {
        $this->billing = $billing;
    }

    /**
     * Returns Entity.
     * The identifier of the Entity that this Billing Modifier applies to.
     */
    public function getEntity(): ?string
    {
        return $this->entity;
    }

    /**
     * Sets Entity.
     * The identifier of the Entity that this Billing Modifier applies to.
     *
     * @maps entity
     */
    public function setEntity(?string $entity): void
    {
        $this->entity = $entity;
    }

    /**
     * Returns Org.
     * The identifier of the Org that this Billing Modifier applies to.
     */
    public function getOrg(): ?string
    {
        return $this->org;
    }

    /**
     * Sets Org.
     * The identifier of the Org that this Billing Modifier applies to.
     *
     * @maps org
     */
    public function setOrg(?string $org): void
    {
        $this->org = $org;
    }

    /**
     * Returns Division.
     * The identifier of the Division that this Billing Modifier applies to.
     */
    public function getDivision(): ?string
    {
        return $this->division;
    }

    /**
     * Sets Division.
     * The identifier of the Division that this Billing Modifier applies to.
     *
     * @maps division
     */
    public function setDivision(?string $division): void
    {
        $this->division = $division;
    }

    /**
     * Returns Partition.
     * The identifier of the Partition that this Billing Modifier applies to.
     */
    public function getPartition(): ?string
    {
        return $this->partition;
    }

    /**
     * Sets Partition.
     * The identifier of the Partition that this Billing Modifier applies to.
     *
     * @maps partition
     */
    public function setPartition(?string $partition): void
    {
        $this->partition = $partition;
    }

    /**
     * Returns Fromentity.
     * The identifier of the Entity that is responsible for paying this Bill on behalf of the Entity
     * specified in the 'entity' field.
     */
    public function getFromentity(): string
    {
        return $this->fromentity;
    }

    /**
     * Sets Fromentity.
     * The identifier of the Entity that is responsible for paying this Bill on behalf of the Entity
     * specified in the 'entity' field.
     *
     * @required
     * @maps fromentity
     */
    public function setFromentity(string $fromentity): void
    {
        $this->fromentity = $fromentity;
    }

    /**
     * Returns Inactive.
     * Whether this resource is marked as inactive.
     *
     * <details>
     * <summary>Valid Values</summary>
     *
     * - `0` - **Active**
     * - `1` - **Inactive**
     *
     * </details>
     */
    public function getInactive(): ?int
    {
        return $this->inactive;
    }

    /**
     * Sets Inactive.
     * Whether this resource is marked as inactive.
     *
     * <details>
     * <summary>Valid Values</summary>
     *
     * - `0` - **Active**
     * - `1` - **Inactive**
     *
     * </details>
     *
     * @maps inactive
     * @factory \PayrixLib\Models\InactiveEnum::checkValue
     */
    public function setInactive(?int $inactive): void
    {
        $this->inactive = $inactive;
    }

    /**
     * Returns Frozen.
     * Whether this resource is marked as frozen.
     *
     * <details>
     * <summary>Valid Values</summary>
     *
     * - `0` - **Not Frozen**
     * - `1` - **Frozen**
     *
     * </details>
     */
    public function getFrozen(): ?int
    {
        return $this->frozen;
    }

    /**
     * Sets Frozen.
     * Whether this resource is marked as frozen.
     *
     * <details>
     * <summary>Valid Values</summary>
     *
     * - `0` - **Not Frozen**
     * - `1` - **Frozen**
     *
     * </details>
     *
     * @maps frozen
     * @factory \PayrixLib\Models\FrozenEnum::checkValue
     */
    public function setFrozen(?int $frozen): void
    {
        $this->frozen = $frozen;
    }

    /**
     * Converts the BillingModifiersPostRequest object to a human-readable string representation.
     *
     * @return string The string representation of the BillingModifiersPostRequest object.
     */
    public function __toString(): string
    {
        return ApiHelper::stringify(
            'BillingModifiersPostRequest',
            [
                'billing' => $this->billing,
                'entity' => $this->entity,
                'org' => $this->org,
                'division' => $this->division,
                'partition' => $this->partition,
                'fromentity' => $this->fromentity,
                'inactive' => $this->inactive,
                'frozen' => $this->frozen
            ]
        );
    }

    /**
     * Encode this object to JSON
     *
     * @param bool $asArrayWhenEmpty Whether to serialize this model as an array whenever no fields
     *        are set. (default: false)
     *
     * @return array|stdClass
     */
    #[\ReturnTypeWillChange] // @phan-suppress-current-line PhanUndeclaredClassAttribute for (php < 8.1)
    public function jsonSerialize(bool $asArrayWhenEmpty = false)
    {
        $json = [];
        $json['billing']       = $this->billing;
        if (isset($this->entity)) {
            $json['entity']    = $this->entity;
        }
        if (isset($this->org)) {
            $json['org']       = $this->org;
        }
        if (isset($this->division)) {
            $json['division']  = $this->division;
        }
        if (isset($this->partition)) {
            $json['partition'] = $this->partition;
        }
        $json['fromentity']    = $this->fromentity;
        if (isset($this->inactive)) {
            $json['inactive']  = InactiveEnum::checkValue($this->inactive);
        }
        if (isset($this->frozen)) {
            $json['frozen']    = FrozenEnum::checkValue($this->frozen);
        }

        return (!$asArrayWhenEmpty && empty($json)) ? new stdClass() : $json;
    }
}
