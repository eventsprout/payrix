...
{
    /**
     * Querying a session resource represents a third-party session who interacts with the API, for example,
     * a referral platform.
     *
     * @param string $id The ID of this session, The session ID.
     ...
     *
     * @return SessionsResponseResult Response from the API call
     *
    ...
    public function getSessionsId(
        ...
        $_reqBuilder = $this->requestBuilder(RequestMethod::GET, '/sessions/{id}')
        ...
        $_resHandler = $this->responseHandler()
            ...
            ->type(SessionsResponseResult::class);
     ...
     * @throws ApiException Thrown if API call fails
     */
    public function deleteSessionsId(
        string $id,
        ?string $token = null,
        ?string $requestToken = null
    ): SessionsResponseResult {
        $_reqBuilder = $this->requestBuilder(RequestMethod::DELETE, '/sessions/{id}')
            ->auth('apiKey', 'sessionKey', 'txnSessionKey')
            ->parameters(
                TemplateParam::init('id', $id),
                HeaderParam::init('token', $token),
    ...
    /**
     * Query a Session.
     * A Session represents a temporary method of authentication to the API. Each Session expires
     ...
     * Keys, Sessions are not permanent.
     *
     * @param string|null $token A single-use, limited authentication layer in addition to an API
     *        Key or Session Key to authorize sensitive operations. <a href="/apis/payrix/dev-int-
     *        guide/references/authentication/auth-tokens" target="_blank">Auth Tokens</a> add
     ...
     *        Format: GET https://{server}.payrix.com/{endpoint}?embed={relatedObject}
     *
     * @return SessionsPostResponseResult Response from the API call
     *
     * @throws ApiException Thrown if API call fails
     */
    public function getSessions(
        ...
        $_reqBuilder = $this->requestBuilder(RequestMethod::GET, '/sessions')
            ->auth('apiKey', 'sessionKey', 'txnSessionKey')
            ->parameters(
                HeaderParam::init('token', $token),
                HeaderParam::init('request-token', $requestToken),
        ...
        $_resHandler = $this->responseHandler()
            ->throwErrorOn('400', ErrorType::init('Error', ErrorFourHundredException::class))
            ->type(SessionsPostResponseResult::class);
     ...
     * Keys, Sessions are not permanent.
     *
     * @param SessionsPostRequest $body Create Session Request
     * @param string|null $token A single-use, limited authentication layer in addition to an API
     *        Key or Session Key to authorize sensitive operations. <a href="/apis/payrix/dev-int-
     *        guide/references/authentication/auth-tokens" target="_blank">Auth Tokens</a> add
     *        security by requiring one-time approval for specific actions.
