...
```php
$entityTermsController = $client->getEntityTermsController();
```
...
[`EntityTermsResponseResult`](../../doc/models/entity-terms-response-result.md)
...
$result = $entityTermsController->getEntityTermsId(
    $id,
    $token,
    $requestToken
);
...
$result = $entityTermsController->getEntityTerms(
    $token,
    $requestToken,
    $search,
    $totals,
...
```php
function postEntityTerms(
    EntityTermsPostRequest $body,
    ?string $token = null,
    ?string $requestToken = null
): EntityTermsResponseResult
```
...
```php
$body = EntityTermsPostRequestBuilder::init()
...
$result = $entityTermsController->postEntityTerms(
    $body,
    $token,
    $requestToken
);
