...
```php
$entryOriginController = $client->getEntryOriginController();
```
...
|  --- | --- | --- | --- |
| `id` | `string` | Template, Required | The Entry Origin ID of this resource. |
...
[`EntryOriginsResponseResult`](../../doc/models/entry-origins-response-result.md)
...
$result = $entryOriginController->getEntryOriginsId(
    $id,
    $token,
    $requestToken
);
...
$result = $entryOriginController->getEntryOrigins(
    $token,
    $requestToken,
    $search,
    $totals,
