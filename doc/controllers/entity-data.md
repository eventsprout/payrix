...
```php
$entityDataController = $client->getEntityDataController();
```
...
|  --- | --- | --- | --- |
| `id` | `string` | Template, Required | The ID of this entity data. |
...
[`EntityDatasResponseResult`](../../doc/models/entity-datas-response-result.md)
...
$result = $entityDataController->getEntityDatasId(
    $id,
    $token,
    $requestToken
);
...
$result = $entityDataController->getEntityDatas(
    $token,
    $requestToken,
    $search,
    $totals,
...
```php
function postEntityDatas(
    EntityDataPostRequest $body,
    ?string $token = null,
    ?string $requestToken = null
): EntityDatasResponseResult
```
...
```php
$body = EntityDataPostRequestBuilder::init(
    't1_ent_641d044866b46287325dd00',
...
$result = $entityDataController->postEntityDatas(
    $body,
    $token,
    $requestToken
);
