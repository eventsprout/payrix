...
```php
$statementEntryController = $client->getStatementEntryController();
```
...
```php
function getStatementEntriesId(
    string $id,
    ?string $token = null,
...
|  --- | --- | --- | --- |
| `id` | `string` | Template, Required | The ID of this statement entry. |
...
[`StatementEntriesResponseResult`](../../doc/models/statement-entries-response-result.md)
...
$result = $statementEntryController->getStatementEntriesId(
    $id,
    $token,
    $requestToken
);
...
$result = $statementEntryController->getStatementEntries(
    $token,
    $requestToken,
    $search,
    $totals,
