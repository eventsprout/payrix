
# Terminal Txns Datas Post Request

## Structure

`TerminalTxnsDatasPostRequest`

## Fields

| Name | Type | Tags | Description | Getter | Setter |
|  --- | --- | --- | --- | --- | --- |
| `terminalTxn` | `string` | Required | The identifier of the TerminalTxn associated with this TerminalTxnDatas resource. | getTerminalTxn(): string | setTerminalTxn(string terminalTxn): void |
| `signature` | `string` | Required | The Base64 encoded image of the signature captured at the time of entering the TerminalTxn. | getSignature(): string | setSignature(string signature): void |

## Example (as JSON)

```json
{
  "terminalTxn": "t1_ttx_67fd4bc0ecb91c5915f3e2e",
  "signature": ""
}
```

