
# Entity Data Post Request

## Structure

`EntityDataPostRequest`

## Fields

| Name | Type | Tags | Description | Getter | Setter |
|  --- | --- | --- | --- | --- | --- |
| `entity` | `string` | Required | The Entity associated with this EntityData. | getEntity(): string | setEntity(string entity): void |
| `tcAcceptSignature` | `string` | Required | The Base64 encoded image of the signature captured at the time of creating the Entity. | getTcAcceptSignature(): string | setTcAcceptSignature(string tcAcceptSignature): void |

## Example (as JSON)

```json
{
  "entity": "t1_ent_641d044866b46287325dd00",
  "tcAcceptSignature": "\""
}
```

